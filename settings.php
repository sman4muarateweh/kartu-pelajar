...
$birthday_var = filter_var(htmlentities($_POST['bd_day']),FILTER_SANITIZE_NUMBER_INT)."/".filter_var(htmlentities($_POST['bd_month']),FILTER_SANITIZE_NUMBER_INT)."/".filter_var(htmlentities($_POST['bd_year']),FILTER_SANITIZE_NUMBER_INT);
...
$darah_var = filter_var(htmlentities($_POST['edit_darah']),FILTER_SANITIZE_STRING);
$agama_var = filter_var(htmlentities($_POST['edit_agama']),FILTER_SANITIZE_STRING);
...
if (!password_verify($EditProfile_current_pass_var,$_SESSION['Password'])) {
    ...
    $update_info_sql = "UPDATE signup SET school= :school_var,work0= :work0_var,work= :work_var,country= :country_var,birthday= :birthday_var,website= :website_var,bio= :bio_var,darah= :darah_var,agama= :agama_var WHERE username= :session_un";
     $update_info = $conn->prepare($update_info_sql);
     ...
     $update_info->bindParam(':darah_var',$darah_var,PDO::PARAM_STR);
     $update_info->bindParam(':agama_var',$agama_var,PDO::PARAM_STR);
    ...
    if (isset($update_info)) {
        ...
        $_SESSION['darah'] = $darah_var;
        $_SESSION['agama'] = $agama_var;
...
if (!password_verify($lang_current_pass_var,$_SESSION['Password'])) {
    ...
    if (isset($update_info)) {
        ...
        $lang_save_result = "<p class='alertGreen'>".lang('changes_saved_seccessfully')."<br></p>";
...
<head>
    <title>konfigurasi | kartu pelajar</title>
    <meta charset="UTF-8">
...
<div class="main_container">
    <center>
<div class="main_container" style="text-align:<?php echo lang('textAlign'); ?>;">
<div style="width: 100%;background: white;margin: 5px;border-radius: 3px;box-shadow: 0px 0px 18px rgba(63, 81, 181, 0.16);">
<ul class="tab" id="settings_ultab">
    ...
    <li>
        <a href="?tc=badge" id="a_badge">
            <p class="tablinks <?php if($tc == 'badge'){echo 'tablinksActive';} ?>"><span class="fa fa-certificate"></span> badge</p>
        </a>
    </li>
    ...
    <li>
        <a href="?tc=language" id="a_language">
            <p class="tablinks <?php if($tc == 'language'){echo 'tablinksActive';} ?>"><span class="fa fa-language"></span> <?php echo lang('language'); ?> (beta)</p>
        </a>
    </li>
    ...
    <li>
        <a href="logout">
            <p  class="tablinks"><span class="fa fa-wheelchair-alt"></span> <?php echo lang('logout'); ?></p>
        </a>
    </li>
</ul>
</div>
...
<div id="Edit_profile" class="tabcontent" style="height: auto;">
    ...
    <p>
      ...
      <p style="background: rgba(247, 81, 81, 0.14); color: #f75151; padding: 15px; border: 1px solid #f75151; border-radius: 3px;"><span class="fa fa-warning"> </span><?php echo lang('personalia_account_note'); ?></p>
    ...
    <select  class="settings_textfield" name="edit_country">
        ...
        if (!empty($_SESSION['country'])) {echo $_SESSION['country'];}else{echo "<?php echo lang('select_country'); ?>";}?></option>
        ...
        echo '<select name="bd_day" class="settings_textfield" style="width: 100%;">';
            for($i = 1; $i <= 31; $i++){
        ...
        echo '</select> ';
        echo '<select name="bd_month" class="settings_textfield" style="width: 100%;">';
            for($i = 1; $i <= 12; $i++){
              $i = str_pad($i, 2, 0, STR_PAD_LEFT);
              if ($exBthD[1] == $i) {
                 echo "<option selected='selected' value='$i'>$i</option>";
        ...
        echo '</select> ';
        echo '<select name="bd_year" class="settings_textfield" style="width: 100%;">';
            for($i = date('Y'); $i >= date('Y', strtotime('-70 years')); $i--){
              if ($exBthD[0] == $i) {
                 echo "<option selected='selected' value='$i'>$i</option>";
              }else{
                echo "<option value='$i'>$i</option>";
        ...
        <textarea style="resize:none;width: 100% height: 100px" dir="auto" class="settings_textfield" placeholder="150" maxlength="124" type="text" name="edit_bio"><?php echo $_SESSION['bio']; ?></textarea></p>
        <p>
        <p class="settings_fieldTitle"><?php echo lang('darah_note'); ?></p>

        <select style="resize:none;width: 100% height: 100px" dir="auto"  type="select" class="settings_textfield"  value="<?php echo $_SESSION['darah']; ?>" placeholder="150" maxlength="124"name="edit_darah" >
          <?php echo $_SESSION['darah']; ?>
          <option></option>
          <option>A</option>
          <option>B</option>
          <option>O</option>
        ...
        <p class="settings_fieldTitle"><?php echo lang('a_note'); ?></p>

        <select style="resize:none;width: 100% height: 100px" dir="auto"  type="select" class="settings_textfield"  value="<?php echo $_SESSION['agama']; ?>" placeholder="150" maxlength="124"name="edit_agama" >
        <?php echo $_SESSION['agama']; ?>
          <option></option>
          <option><?php echo lang('a_konghucu'); ?></option>
          <option><?php echo lang('a_buddha'); ?></option>
          <option><?php echo lang('a_hindu'); ?></option>
          <option><?php echo lang('a_khatolik'); ?></option>
          <option><?php echo lang('a_islam'); ?></option>
          <option><?php echo lang('a_kristen'); ?></option>
        </select>
<p>
        <div style="background: #e9ebee; border-radius: 3px; padding: 15px;">
...
<?php break; ?>
<?php case 'badge': ?>
<div id="Badge" class="tabcontent" style="height: auto;">
         <p align="center" id="qr_code_save_result"><?php echo $lang_save_result; ?></p>
    <form action="" method="post">
    <p>Apa itu lencana (badge)? </hp>
    <p>lencana adalah sistem opsional tambahan yang ada pada platform yang mana sebagai penunjang keterangan profilmu,lencana dapat didapatkan dengan memenuhi jika mengikuti persyaratan,seperti berikut..</p>

    <p><img src="imgs/badge/alpha.png" border=3 height=74px width=74px data-toggle='tooltip' title='Genezisitator'></img>Genezisitator <br>Genezisitator is badge to appriciate u who is
using this platform at alpha version
such as adam and eve in eden,someone with this badge
had 4% rate added to the post u posted became more trending than other user,thank u :)
</p>

<p><img src="imgs/badge/bugB.png" border=3 height=74px width=74px data-toggle='tooltip' title='Bug Katchy'></img>Bug Katchy  <br>
adalah lencana jika bagi yang memenumukan bug pada sistem dan tentu saja kamu diapreasiasi oleh pengembang
</p>

<p><img src="imgs/badge/police.png" border=3 height=74px width=74px data-toggle='tooltip' title='Poluziation '></img>Poluziation <br>
adalah lencana jika adalah moderator diplatform ini yang mana bertugas sebagai yang dapat menghapus atau mengedit postingan orang lain
</p>

<p><img src="imgs/badge/smafourmute.png" border=3 height=74px width=74px data-toggle='tooltip' title='SMA4mute Member '></img>SMAN4-MT member <br>
lencana ini hanya dapat didapat jika kamu telah terverifikasi sebagai anggota <br>SMAN4 Muara Teweh atau alumninya,orang dengan lencana ini adalah resmi dan terverifikasi sebagai salah satu anggota SMAN4 Muara Teweh</p>
...
<div id="Language" class="tabcontent" style="height: auto;">
    ...
    <form action="" method="post">
    <div style="background: #e9ebee; border-radius: 3px; padding: 15px;">
    <p style="background: rgba(247, 81, 81, 0.14); color: #f75151; padding: 15px; border: 1px solid #f75151; border-radius: 3px;"><?php echo lang('translate_note'); ?></p>
        <p>
        <select class="settings_textfield" name="edit_language">
        <option <?php if($_SESSION['language'] == "Indonesian"){ echo "selected";} ?> >Indonesian</option>
...
<?php default: ?>
    <div id="General" class="tabcontent" style="height: auto;">
        ...
        <form action="" method="post">
        <div style="background: #e9ebee; border-radius: 3px; padding: 15px;">
        <p style="background: rgba(247, 81, 81, 0.14); color: #f75151; padding: 15px; border: 1px solid #f75151; border-radius: 3px;"><?php echo lang('general_account_note'); ?></p>
        </div>
            <p>
            ...
            <p>
            <p class="settings_fieldTitle"><?php echo lang('new_password'); ?> <span><?php echo lang('required'); ?></span></p>
            <input class="settings_textfield" type="password" name="new_pass" /></p>
            <p>
            <p class="settings_fieldTitle"><?php echo lang('confirm_new_password'); ?> <span><?php echo lang('required'); ?></span></p>
            ...
            <p class="settings_fieldTitle"><?php echo lang('gender'); ?> <span><?php echo lang('required'); ?></span></p>
             ...
             <option <?php if($_SESSION['gender'] == "Netral"){ echo "selected";} ?> ><?php echo lang('netral'); ?></option>
             </select>
             </p>
             <div style="background: #e9ebee; border-radius: 3px; padding: 15px;">
